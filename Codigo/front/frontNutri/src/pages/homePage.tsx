...
import { getPatientvisits, getPatientvisitsMensais, getSchedule, getSumaryData } from "../services/appointment.service";
...
export const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const getPatientvisit = async () => {
      const { data, error } = await getPatientvisitsMensais(parseInt(selectedYear));
  ...
  const divideDataIntoSemesters = (data: MonthData) => {
    const firstSemesterMonths = ["janeiro", "fevereiro", "março", "abril", "maio", "junho"];
    const secondSemesterMonths = ["julho", "agosto", "setembro", "outubro", "novembro", "dezembro"];
        ...
        <Row gutter={16} style={{ marginTop: 16 }}>
          <Col span={16}>
            <div className="header-actions">
              <Select defaultValue="2024" onChange={(value) => getPatientvisit(value)} className="select-filter">
            ...
            <Card bordered={false} className="chart-card">
              <div className="patient-visit">
                ...
                <Select defaultValue="1" onChange={(value) => changeSem(value)} className="select-filter">
